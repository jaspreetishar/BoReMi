...
def boremi(total_tabs, list_for_all_data_types, 
    ...
    for number_of_tab in range(len(total_tabs)):
        ...
        dict_for_scale_bar_pixel_size["sliders"].append(Slider(start=0.01, end=3, value=scale_bar, step=0.01, 
        ...
        dict_for_data_save_button["save_buttons"][number_of_tab].js_on_event(ButtonClick, CustomJS(
            args=dict(source_data=list_for_data_table[number_of_tab], 
                    ...
                    px_size=dict_for_he_pixel_size["sliders"][number_of_tab], source_img=source_img,
                     ...
                     multichoice=dict_for_multichoice["multichoice"][number_of_tab],
                     clusters=list_for_storing_total_clusters_info_in_each_file[number_of_tab]),
            code="""

                // for saving a .csv file containing updated x-y coordinates
                
                multichoice.value = clusters;
                
                var data = source_data.source.data;
                var x = data['x'];
                var y = data['y'];
                ...
                var file = new Blob([source_img.data['imgs'][N]], {type: 'image/octet-stream'});
                var elem = window.document.createElement('a');
                elem.href = window.URL.createObjectURL(file);
                elem.download = 'Updated_image';
                document.body.appendChild(elem);
                elem.click();
                document.body.removeChild(elem); 
